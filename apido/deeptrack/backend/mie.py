...
def stratified_mie_coefficients(m, a, L):
    ...
    for n in range(L):
            ...
            for j in range(2 * n_layers):
                ...
                if np.mod(i, 2) == 0:
                    ...
                    if (j < 2 * n_layers - 1) and ((j == 0) or (np.mod(j, 2) == 1)):
                        A[i, j] = D.dricbesj(n + 1, m[p] * a[q])
                    elif np.mod(j, 2) == 0:
                        A[i, j] = D.dricbesy(n + 1, m[p] * a[q])
                    else:
                        A[i, j] = D.dricbesj(n + 1, a[q])
                    ...
                    if (j < 2 * n_layers - 1) and ((j == 0) or (np.mod(j, 2) == 1)):
                        C[i, j] = D.ricbesj(n + 1, m[p] * a[q])
                    elif np.mod(j, 2) == 0:
                        C[i, j] = D.ricbesy(n + 1, m[p] * a[q])
                    else:
                        C[i, j] = D.ricbesj(n + 1, a[q])
        ...
        B = A + 0j
        B[-2, -1] = D.dricbesh(n + 1, a[-1])
        B[-1, -1] = D.ricbesh(n + 1, a[-1])
        an[n] = np.linalg.det(A) / np.linalg.det(B)

        D = C + 0j
        D[-2, -1] = D.dricbesh(n + 1, a[-1])
        D[-1, -1] = D.ricbesh(n + 1, a[-1])
